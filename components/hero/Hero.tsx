    ...
    <section ref={containerRef} id="hero" className="relative min-h-[100dvh] w-full flex flex-col items-center justify-center overflow-hidden bg-transparent p-6 md:p-12 lg:p-16">
      ...
      <div className="max-w-7xl w-full flex flex-col items-center z-10 relative">
        ...
        <motion.div 
          ...
          className="w-full flex justify-between items-start mb-16 px-4"
        >
          <div className="flex flex-col gap-1">
            <span className="text-[10px] font-black uppercase tracking-[0.5em] text-purple-500">Portfolio</span>
            <span className="text-[9px] font-bold uppercase tracking-[0.3em] text-slate-400 dark:text-white/20">Ed. 2026</span>
          </div>
          <div className="hidden md:block">
            <span className="text-[10px] font-bold uppercase tracking-[0.4em] text-slate-400 dark:text-white/20">Available for Projects</span>
          </div>
        </motion.div>

        {/* MAIN VISUAL BOX (Framed Scene) */}
        <motion.div
          initial={{ opacity: 0, scale: 0.95 }}
          animate={{ opacity: 1, scale: 1 }}
          transition={{ duration: 1.2, ease: "easeOut" }}
          className="relative w-full aspect-[16/10] md:aspect-[21/9] max-h-[60vh] rounded-[3.5rem] overflow-hidden bg-slate-50 dark:bg-black/40 border border-slate-200 dark:border-white/5 shadow-2xl mb-12 flex items-center justify-center px-6 md:px-12"
        >
          {/* CONTENT INSIDE BOX */}
          <div className="flex flex-col items-center text-center gap-8 max-w-4xl relative z-10">
            <motion.h1 
              ...
              className="text-5xl md:text-8xl lg:text-[10rem] font-black text-slate-900 dark:text-white leading-[0.85] tracking-tighter uppercase italic"
            >
              I AM <br />
      ...
      <motion.div 
        ...
        className="absolute bottom-10 lg:bottom-24 left-1/2 -translate-x-1/2 flex flex-col items-center gap-4 z-10"
      >
        <span className="text-[9px] font-bold uppercase tracking-[0.5em] text-slate-400 dark:text-white/20 whitespace-nowrap">Scroll to Explore</span>
